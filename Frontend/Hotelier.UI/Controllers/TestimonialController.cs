using Hotelier.UI.Models.Testimonial;
...
namespace Hotelier.UI.Controllers
{
    public class TestimonialController : Controller
    {
        ...
        public async Task<IActionResult> AddTestimonial(TestimonialViewModel model)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5041/api/Testimonial", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateTestimonial(TestimonialViewModel model)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(model);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:3523/api/Testimonial/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
